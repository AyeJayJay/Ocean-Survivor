...
const THEMES: Theme[] = [
  ...
  },
  {
    ...
    rayColor: "rgba(0,210,140,",
    bubbleRgb: "0,200,140",
    floorTop: "#043a24",
    floorBot: "#021410",
    floorAccent: "rgba(0,180,100,0.25)",
    topColors: ["#1a3a10", "#122a08", "#234e18", "#0c1e08"],
    botColors: ["#3a5a20", "#2c4818", "#4c6e2c", "#1e3010"],
    topBranchColors: ["#7a9440", "#5a7428", "#9ab455"],
    botBranchColors: ["#6a8438", "#4e6820", "#88a448"],
    particleColors: ["#7a9440", "#9ab450", "#c8b860", "#4a6820"],
    scoreGlow: "rgba(120,160,60,0.9)",
    ambient: "rays",
    ambientCount: 6,
  },
  {
...
function drawTrashItem(
  ...
  switch (item.type) {
    ...
    case "bag": {
      ...
      ctx.beginPath();
      ctx.moveTo(0, -14);
      ctx.bezierCurveTo(10, -10, 14, 0, 10, 10);
      ...
      ctx.beginPath();
      ctx.ellipse(0, -14, 3, 2, 0, 0, Math.PI * 2);
      ctx.fill();
      break;
    }
    case "can": {
...
function drawBackground(
  ...
  floorGrad.addColorStop(1, theme.floorBot);
  ctx.fillStyle = floorGrad;
  ctx.beginPath();
  ctx.moveTo(0, h - 30);
  for (let i = 0; i <= w; i += 20) {
    ctx.lineTo(i, h - 30 + Math.sin((i + scrollX * 0.3) * 0.06) * 10);
  }
  ctx.lineTo(w, h);
  ctx.lineTo(0, h);
  ctx.closePath();
...
function drawPillarDecorations(
  ...
  if (themeIdx === 3) {
    ...
    for (let s = 0; s < numShards; s++) {
      ...
      ctx.fillStyle = branchColors[s % 3];
      ctx.globalAlpha = 0.7;
    ...
    for (let v = 0; v < 4; v++) {
      ...
      ctx.strokeStyle = branchColors[v % 3];
      ctx.shadowColor = branchColors[v % 3];
      ctx.shadowBlur = 10;
      ctx.globalAlpha = pulse;
      ctx.beginPath();
      ctx.moveTo(vx, y);
      ...
      while (cy < y + h) {
        cy += 18;
        ctx.lineTo(
          vx + Math.sin(cy * 0.15 + tick * 0.03) * 5,
          Math.min(cy, y + h),
        );
      }
      ctx.stroke();
    ...
    for (let b = 0; b < numBranches; b++) {
      ...
      ctx.beginPath();
      ctx.moveTo(bx, by);
      ctx.lineTo(bx + sway, by + (fromTop ? 1 : -1) * 25);
      ctx.stroke();
      ctx.lineWidth = 3;
      const dir = fromTop ? 1 : -1;
      for (let i = 0; i < 2; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(bx + sway * 0.5, by + dir * 12);
        ctx.lineTo(
          bx + sway * 0.5 + Math.sin(angle) * 14,
          by + dir * 12 + Math.cos(angle) * dir * 14,
        );
        ctx.stroke();
      }
      ctx.fillStyle = color;
      ...
      [
        [bx + sway, by + dir * 27],
        [bx + sway * 0.5 + Math.sin(-0.6) * 15, by + dir * 27],
        [bx + sway * 0.5 + Math.sin(0.6) * 15, by + dir * 27],
      ].forEach(([cx, cy]) => {
        ctx.beginPath();
...
function drawPillar(
  ...
  if (fromTop) {
    ...
    for (let i = 0; i < spikes; i++) {
      const sx = x + i * sw;
      const pk = 10 + Math.sin(i * 1.7 + tick * 0.01) * 5;
      ctx.lineTo(sx + sw / 2, edgeY + pk);
...
function drawUI(
  ...
  for (let i = 0; i < totalDots; i++) {
    ctx.beginPath();
    ctx.arc(startX + i * 16, dotY, i === themeIdx ? 5 : 3, 0, Math.PI * 2);
...
function drawOverlay(
  ...
  if (state === "idle") {
    ...
    ctx.beginPath();
    ctx.roundRect(w / 2 - 160, h / 2 - 152, 320, 272, 24);
    ctx.fill();
    ctx.fillStyle = theme.scoreGlow
      .replace("rgba", "rgb")
      .replace(/,[\d.]+\)/, ")");
    ...
    ctx.textAlign = "center";
    ctx.fillText("SEA TURTLE", w / 2, h / 2 - 105);
    ctx.shadowBlur = 0;
    ctx.fillStyle = "#4dc47a";
    ctx.font = "bold 20px 'Segoe UI', sans-serif";
    ctx.fillText("DASH", w / 2, h / 2 - 77);
    ctx.font = "54px serif";
    ctx.fillText("🐢", w / 2, h / 2 - 27);
    ctx.fillStyle = "rgba(255,255,255,0.9)";
    ctx.font = "18px 'Segoe UI', sans-serif";
    ctx.fillText("Tap or press SPACE", w / 2, h / 2 + 45);
    ctx.fillStyle = "rgba(150,210,255,0.7)";
    ctx.font = "14px 'Segoe UI', sans-serif";
    ctx.fillText("to start swimming!", w / 2, h / 2 + 70);
    if (best > 0) {
      ctx.fillStyle = "#f0c060";
      ctx.font = "14px 'Segoe UI', sans-serif";
      ctx.fillText(`Best: ${best}`, w / 2, h / 2 + 96);
    }
  }
  if (state === "dead") {
    ...
    ctx.textAlign = "center";
    ctx.fillText("OH NO!", w / 2, h / 2 - 110);
    ctx.font = "48px serif";
    ...
    if (trashCount > 0) {
      ctx.fillStyle = "rgba(150,220,255,0.8)";
      ctx.font = "13px 'Segoe UI', sans-serif";
      ctx.fillText(`Ocean cleaned: ${trashCount} pieces 🧹`, w / 2, h / 2 + 18);
    }
    ...
    ctx.fillStyle = "rgba(255,255,255,0.9)";
    ctx.font = "18px 'Segoe UI', sans-serif";
    ctx.fillText("Tap or press SPACE", w / 2, h / 2 + 76);
    ctx.fillStyle = "rgba(150,210,255,0.7)";
    ctx.font = "14px 'Segoe UI', sans-serif";
    ctx.fillText("to try again", w / 2, h / 2 + 100);
  }
}
...
export default function Game() {
  ...
  const [donateResult, setDonateResult] = useState<"success" | "cancel" | null>(
    null,
  );
  ...
  const initAmbientForTheme = useCallback((idx: number, saveOld = false) => {
    ...
    if (theme.ambient === "jellyfish")
      jelliesRef.current = Array.from({ length: theme.ambientCount }, () => ({
        ...
        r: 10 + Math.random() * 18,
        color: ["#b060ff", "#4080ff", "#ff60c0", "#60d0ff"][
    ...
    if (theme.ambient === "angler")
      anglerLightsRef.current = Array.from(
        { length: theme.ambientCount },
        () => ({
          x: Math.random() * CANVAS_WIDTH,
          y: Math.random() * CANVAS_HEIGHT,
          phase: Math.random() * Math.PI * 2,
          r: 6 + Math.random() * 10,
          color: ["#00ffb0", "#00e0ff", "#ff00c0", "#80ff00"][
            Math.floor(Math.random() * 4)
          ],
        }),
      );
  ...
  const resetGame = useCallback(() => {
    ...
    lastTrashTimeRef.current = 0;
    trashItemsRef.current = [];
    floatTextsRef.current = [];
    trashCountRef.current = 0;
    ...
    setReviveUsed(false);
    prevJelliesRef.current = [];
    prevEmbersRef.current = [];
    prevFlakesRef.current = [];
    prevAnglerLightsRef.current = [];
  ...
  useEffect(() => {
    analytics.track("session_start", {
      is_new_user: adFrequencyManager.newUser,
      session_number: adFrequencyManager.sessionNumber,
    });
    initBubbles();
    initAmbientForTheme(0);
    ...
    function gameLoop(timestamp: number) {
      ...
      if (curTheme.ambient === "jellyfish")
        jelliesRef.current.forEach((j) => {
          j.y -= j.speed;
          if (j.y + j.r * 2 < 0) {
            j.y = CANVAS_HEIGHT + j.r;
            j.x = Math.random() * CANVAS_WIDTH;
          }
        });
      ...
      if (state === "playing") {
        ...
        if (turtle.y + TURTLE_SIZE / 2 > CANVAS_HEIGHT - 20) {
          ...
          analytics.track("game_over", {
            ...
            death_cause: "floor",
            theme_index: themeIdxRef.current,
          });
          setUiState("dead");
        }
        ...
        if (timestamp - lastObstacleTimeRef.current > BASE_INTERVAL) {
          const gap = BASE_GAP;
          const minGapY = gap / 2 + 60;
          const maxGapY = CANVAS_HEIGHT - gap / 2 - 60;
        ...
        if (
          timestamp - lastTrashTimeRef.current >
          TRASH_INTERVAL + Math.random() * 1000
        ) {
          const types: TrashType[] = ["bottle", "bag", "can", "straw"];
          const type = types[Math.floor(Math.random() * types.length)];
          const y = 120 + Math.random() * 380;
          trashItemsRef.current.push({
        ...
        obstaclesRef.current = obstaclesRef.current.filter((obs) => {
          ...
          if (!obs.scored && obs.x + OBSTACLE_WIDTH < TURTLE_X) {
            ...
            if (scoreRef.current >= nextMilestoneRef.current) {
              ...
              milestoneAltIdxRef.current =
                (milestoneAltIdxRef.current + 1) % MILESTONE_PATTERN.length;

              const newThemeIdx = Math.min(
                themeStepRef.current,
                THEMES.length - 1,
              );
          ...
          const inXRange =
            TURTLE_X + hitR > obs.x && TURTLE_X - hitR < obs.x + OBSTACLE_WIDTH;
          if (inXRange) {
            const topEdge = obs.gapY - obs.gap / 2;
            const botEdge = obs.gapY + obs.gap / 2;
        ...
        // Gameplay banner: hide when any obstacle is within 220px of the turtle
        {
          ...
          const isNear = obstaclesRef.current.some(
            (obs) =>
              obs.x - TURTLE_X < dangerZone &&
              obs.x + OBSTACLE_WIDTH > TURTLE_X - TURTLE_SIZE,
          );
        ...
        trashItemsRef.current = trashItemsRef.current.filter((t) => {
          t.x -= TRASH_SPEED;
          const dx = TURTLE_X - t.x;
          const dy = turtle.y - (t.y + Math.sin(tick * 0.025 + t.phase) * 8);
        ...
        if (themeNameAlphaRef.current > 0)
          themeNameAlphaRef.current = Math.max(
            0,
            themeNameAlphaRef.current - 0.007,
          );
      }
      ...
      if (wipeX !== null && wipeX > 0) {
        ...
        ctx.save();
        ctx.beginPath();
        ctx.rect(wipeX, 0, CANVAS_WIDTH - wipeX, CANVAS_HEIGHT);
        ctx.clip();
        drawBackground(
          ctx,
          CANVAS_WIDTH,
      ...
      if (showTurtle)
        drawTurtle(ctx, TURTLE_X, turtle.y, turtle.angle, TURTLE_SIZE, tIdx);
